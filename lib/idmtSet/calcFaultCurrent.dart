...
class FaultCurrentScreen extends StatefulWidget {
  _FaultCurrentState createState() => _FaultCurrentState();
}

class _FaultCurrentState extends State<FaultCurrentScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: new Text("Calculate Fault Current"),
      ),
      body: new Container(
  ...
  Widget buildResultText() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          new Text("Fault current required to operate relay in above mentioned time: " + _requiredFaultCurrent),
        ],
      ),
    );
  }
    ...
    return ButtonBar(children: <Widget>[
      (RaisedButton(
        ...
        onPressed: () {
          ...
          double finalFaultCurrent = _setValue * (pow(intermedeateWithP, 1/_selectCurve.aValue));

          setState(() {
            _requiredFaultCurrent = finalFaultCurrent.toStringAsFixed(2) + " A";
          });
        },
        child: Container(
          height: 50.0,
